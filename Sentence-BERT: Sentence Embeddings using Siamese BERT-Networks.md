...
句子嵌入 (Sentence Embedding) 是將文本轉換為**固定維度數值向量**的技術。

核心目標：**語意相似的句子，在向量空間中也很靠近**，方便後續的語意運算。

---

## 2. Pooling 策略：從 BERT 輸出產生句子向量
...
網路架構與目標函數緊密耦合，**訓練資料的形式決定了兩者的選擇**。

### 3.1 Siamese 架構 + Classification 目標函數

適用資料：有標籤的分類資料（如 NLI: entailment / contradiction / neutral）
...
### 3.2 Siamese 架構 + Regression 目標函數

適用資料：有連續相似度分數的資料（如 STS，分數範圍 0～5）
...
Paper 採用 **Spearman 相關性 $\rho$**（而非 Pearson），因為 Pearson 相關性對 STS 任務有嚴重缺陷。
...
AFS 語料庫：從社群媒體標註 6000 個論點句子對，涵蓋槍枝管制、同性婚姻、死刑三個主題，標註尺度 0（不相關）～ 5（完全相同）。
...
## 6. 過去方法的問題（BERT / RoBERTa）

| | BERT（Cross-Encoder） |
|---|---|
| **優勢** | 在 sentence-pair regression 任務（如 STS）表現出色 |
| **缺點** | 兩個句子都需餵入網路，計算開銷龐大，不適用於 Clustering、大規模語意搜尋等任務 |

---

## 7. Big Picture：應用於 Opinion Dynamics 研究

利用 SBERT 作為 Agent 意見動態模擬的核心組件：

```
Textual Opinion
      ↓
Opinion Classifier → Numeric Rating
      ↓
SBERT → 計算 Agents 間 Opinion 的最高相似性
      ↓
K-NN + Game Theory → 決定 Agent 要與哪個 Agent 交流
```
